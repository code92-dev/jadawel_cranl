  ...
  computed: {
    ...
    handleStyle() {
      if (this.dragging && this.dragMouseX !== null) {
        return { insetInlineStart: this.dragMouseX + 'px' }
      }
      return { insetInlineStart: this.leftWidth + 'px' }
    },
    gripStyle() {
    ...
    tooltipStyle() {
      const y = this.dragging ? this.dragMouseY : this.hoverMouseY
      if (y === null) return {}
      return { top: y - 10 + 'px', insetInlineStart: '16px', transform: 'none' }
    },
    snapLineStyle() {
      if (this.snapLineOffset === null) return {}
      return { insetInlineStart: this.snapLineOffset + 'px' }
    },
  },
  mounted() {
  ...
  methods: {
    ...
    startDrag(event) {
      ...
      const onMove = (e) => {
        ...
        const relativeX = getInlinePointerPosition(e.clientX, gridRect, rtl)
