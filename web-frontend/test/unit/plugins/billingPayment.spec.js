...
test('browser payment uses the persisted payment identity and server quote', async () => {
  const transport = vi
    .fn()
  ...
  expect(transport.mock.calls[0][0]).toBe('https://api.moyasar.com/v1/payments')
  const body = JSON.parse(transport.mock.calls[0][1].body)
  expect(body.given_id).toBe('given-1')
  expect(body.amount).toBe(5000)
  expect(body.source.month).toBe(12)
  expect(body.source.year).toBe(2030)
  expect(body.metadata).toEqual({ billing_order: 'order-1' })
})
