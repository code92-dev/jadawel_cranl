...
@pytest.mark.django_db
@patch("jadawel.contrib.database.rows.signals.rows_updated.send")
def test_update_row_by_id(send_mock, data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.rows.signals.before_rows_update.send"
    ) as before_send_mock:
        handler.update_row_by_id(
            user=user,
            table=table,
...
@pytest.mark.django_db
@patch("jadawel.ws.tasks.broadcast_to_users.delay")
@patch("jadawel.contrib.database.table.signals.table_updated.send")
@patch("jadawel.contrib.database.rows.signals.rows_created.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.rows.signals.rows_updated.send")
@patch("jadawel.contrib.database.rows.signals.before_rows_update.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.rows.signals.row_orders_recalculated.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.rows.signals.rows_created.send")
@patch("jadawel.contrib.database.views.handler.ViewHandler.field_value_updated")
@patch(
    "jadawel.contrib.database.search.handler.SearchHandler.schedule_update_search_data"
)
...
def test_rows_created_is_not_sent_if_there_are_no_rows_to_create(data_fixture):
    user = data_fixture.create_user()
    table = data_fixture.create_database_table()
    with patch("jadawel.contrib.database.rows.signals.rows_created.send") as mock:
