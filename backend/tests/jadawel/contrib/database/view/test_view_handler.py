...
from jadawel.contrib.database.views.actions import UpdateViewDefaultValuesActionType
...
from jadawel.contrib.database.views.view_ownership_types import (
...
from jadawel.core.db import get_collation_name
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_created.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_updated.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_deleted.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_created.send")
@patch("jadawel.contrib.database.views.signals.views_reordered.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_field_options_updated.send")
...
def test_field_type_single_field_num_queries(data_fixture, django_assert_num_queries):
    ...
    # +3 over upstream, one query per fork-registered cleanup participant:
    # the two arabase row-coloring value providers (single select DELETE,
    # conditional SELECT) and the arabase kanban view type (one merged UPDATE
    # clearing both of its stale field references), the same per-type cost
    # the gallery view type already adds.
...
def test_field_type_changed_two_fields_num_queries(
    ...
    # Keep this bounded independently of how many fields are changed.
    # +3 over upstream: see test_field_type_single_field_num_queries; the
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_filter_created.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_filter_updated.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_filter_deleted.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_sort_created.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_sort_updated.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_sort_deleted.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.rows.signals.rows_created.send")
...
@pytest.mark.enable_signals(
    "jadawel.contrib.database.search.tasks.schedule_update_search_data.delay",
    "jadawel.contrib.database.search.tasks.update_search_data.delay",
)
...
def test_create_view_ownership_type(data_fixture):
    ...
    with patch(
        "jadawel.contrib.database.views.registries.view_ownership_type_registry.registry",
        ownership_types,
    ):
        user = data_fixture.create_user()
        table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.enable_signals(
    "jadawel.contrib.database.views.tasks.update_view_index.delay"
)
...
@patch("jadawel.contrib.database.views.tasks.update_view_index.delay")
@pytest.mark.django_db(transaction=True)
@pytest.mark.enable_signals(
    "jadawel.contrib.database.views.tasks.update_view_index.delay"
)
...
def test_update_index_replaces_index_with_diff_collation(settings, data_fixture):
    with patch("jadawel.core.db.get_collation_name", new=lambda: None):
        user = data_fixture.create_user()
        table = data_fixture.create_database_table(user=user)
        text_field = data_fixture.create_text_field(user=user, table=table)
        handler = ViewHandler()
    ...
    with patch(
        "jadawel.core.db.get_collation_name", new=lambda: settings.EXPECTED_COLLATION
    ):
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_group_by_created.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_group_by_updated.send")
...
@pytest.mark.django_db
@patch("jadawel.contrib.database.views.signals.view_group_by_deleted.send")
