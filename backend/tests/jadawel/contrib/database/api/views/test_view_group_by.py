...
from jadawel.contrib.database.views.constants import MAX_GROUP_BYS
from jadawel.contrib.database.views.handler import ViewHandler
from jadawel.contrib.database.views.models import ViewGroupBy
...
@pytest.mark.django_db
def test_create_view_group_by_rejects_sixth_group_by(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    table = data_fixture.create_database_table(user=user)
    view = data_fixture.create_grid_view(table=table)
    ...
    groupable = [
        ...
        for index in range(MAX_GROUP_BYS + 1)
    ]

    for field in groupable[:MAX_GROUP_BYS]:
        response = api_client.post(
            reverse("api:database:views:list_group_bys", kwargs={"view_id": view.id}),
            {"field": field.id, "order": "ASC"},
            format="json",
    ...
    response = api_client.post(
        reverse("api:database:views:list_group_bys", kwargs={"view_id": view.id}),
        {"field": groupable[MAX_GROUP_BYS].id, "order": "ASC"},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_400_BAD_REQUEST
    assert response.json()["error"] == "ERROR_VIEW_GROUP_BY_LIMIT_REACHED"
    assert view.viewgroupby_set.count() == MAX_GROUP_BYS
