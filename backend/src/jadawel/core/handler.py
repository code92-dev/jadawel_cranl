...
from jadawel.core.cache import get_cached_settings, set_cached_settings
...
class CoreHandler(metaclass=jadawel_trace_methods(tracer, exclude="clear_context")):
    ...
    def get_settings(self) -> Settings:
        ...
        on the ``Settings`` model (see ``jadawel.core.receivers``).
    ...
    def sync_templates(
        ...
        sync_templates_import_export_config = ImportExportConfig(
            ...
            # Without reducing disk space usage Jadawel after first time install
        ...
        if failed_templates:
            logger.warning(
                "Synced {} of {} templates. Skipped {} that could not be imported: {}",
