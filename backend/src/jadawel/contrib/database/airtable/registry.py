...
from jadawel.contrib.database.airtable.config import AirtableImportConfig
from jadawel.contrib.database.airtable.constants import (
    AIRTABLE_ASCENDING_MAP,
    AIRTABLE_BASEROW_COLOR_MAPPING,
)
from jadawel.contrib.database.airtable.exceptions import (
    AirtableSkipCellValue,
    AirtableSkipFilter,
)
from jadawel.contrib.database.airtable.import_report import (
...
from jadawel.contrib.database.airtable.models import DownloadFile
...
class AirtableColumnType(Instance):
    def to_jadawel_field(
        ...
        """
        Converts the raw Airtable column to a Jadawel field object. It should be
        possible to pass this object directly into the FieldType::export_serialized
        method to convert it to the Jadawel export format.
        ...
        :param import_report: Used to collect what wasn't imported to report to the
            user.
        :return: The Jadawel field type related to the Airtable column. If None is
    ...
    def to_jadawel_export_serialized_value(
        ...
        :param import_report: Used to collect what wasn't imported to report to the
            user.
        :return: The converted value is Jadawel export format.
        """
    ...
    def add_import_report_failed_if_default_is_provided(
        ...
        if default:
            ...
            import_report.add_failed(
                ...
                f"{default} was dropped because that's not supported in Jadawel.",
            )
...
class AirtableColumnTypeRegistry(Registry):
    ...
    def from_airtable_column_to_serialized(
        ...
        """
        Tries to find a Jadawel field that matches that raw Airtable column data. If
        None is returned, the column is not compatible with Jadawel and must be ignored.
        ...
        :param import_report: Used to collect what wasn't imported to report to the
            user.
        :return: The related Jadawel field and AirtableColumnType that should be used
            for the conversion.
        """
...
class AirtableViewType(Instance):
    ...
    def get_sorts(
        ...
        """
        Maps the sorts from the raw Airtable view data to a list of Jadawel
    ...
    def get_group_bys(
        ...
        """
        Maps the group bys from the raw Airtable view data to a list of Jadawel
    ...
    def get_filter(
        ...
        """
        This method converts a raw airtable filter object into a Jadawel filter object
    ...
    def get_filters(
        ...
        converts it to two flat lists containing the Jadawel ViewFilter and
    ...
    def get_color_definitions_decoration(
        ...
        for color_definition in color_definitions:
            ...
            # Pop the first group because that shouldn't be in Jadawel, and the type is
            # defined on the view.
    ...
    def get_decorations(
        ...
        """
        Converts the raw Airtable color config into matching Jadawel view decorations.
        """
    ...
    def _check_personal_or_locked(
        ...
        if raw_airtable_view.get("lock", None):
            import_report.add_failed(
                ...
                f"it Jadawel does not support this yet.",
            )
    ...
    def to_serialized_jadawel_view(
        ...
        # that we can find the order for the Jadawel view.
        ...
        for view in raw_airtable_table["viewOrder"]:
            if view in raw_airtable_table["viewSectionsById"]:
                ...
                # Jadawel doesn't support sections, but we can prepend the name of the
                # section.
    ...
    def prepare_view_object(
        ...
        default depending on the Jadawel view support for it.
        ...
        :param import_report: Used to collect what wasn't imported to report to the
            user.
        :return: The Jadawel view type related to the Airtable column. If None is
...
class AirtableViewTypeRegistry(Registry):
    ...
    def from_airtable_view_to_serialized(
        ...
        """
        Tries to find a Jadawel view that matches that raw Airtable view data. If
        None is returned, the view is not compatible with Jadawel and must be ignored.

        :param field_mapping: A dict containing all the imported fields.
        :param row_id_mapping: A dict mapping the Airable row IDs to Jadawel row IDs
            per table ID.
        :param raw_airtable_table: The raw Airtable table data related to the column.
        ...
        :param import_report: Used to collect what wasn't imported to report to the
            user.
        :return: The related Jadawel view and AirtableViewType that should be used
            for the conversion.
        """
...
class AirtableFilterOperator(Instance):
    def to_jadawel_filter_and_value(
        ...
        """
        Converts the given Airtable value into the matching Jadawel filter type and
        correct value.
        ...
        :raises AirtableSkipFilter: If no compatible Jadawel filter can be found.
        :return: The matching Jadawel filter type and value.
        """
...
# throughout the whole Jadawel application to add a new airtable column type.
